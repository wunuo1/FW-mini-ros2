...
namespace yhs
{
      ...
      : node_(node), if_name_("can0"), can_socket_(-1)
  {
    ...
    geometry_ctrl_cmd_subscriber_ = node_->create_subscription<geometry_msgs::msg::Twist>(
        "cmd_vel",
        1,
        std::bind(&CanControl::geometry_ctrl_cmd_callback, this, std::placeholders::_1));
  ...
  void CanControl::geometry_ctrl_cmd_callback(const geometry_msgs::msg::Twist::SharedPtr ctrl_cmd_msg)
  {

    const short ctrl_cmd_x_linear = ctrl_cmd_msg->linear.x * 1000;
    const short ctrl_cmd_z_angular = (ctrl_cmd_msg->angular.z * 180 / M_PI) * 100;
    const short ctrl_cmd_y_linear = ctrl_cmd_msg->linear.y * 1000;
    unsigned char gear;
    if (ctrl_cmd_y_linear == 0){
      gear = '6';
    } else {
      gear = '8';
    }
  ...
  bool CanControl::wait_for_can_frame()
  {
    ...
    if (ret == -1)
    {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("yhs_can_control_node"), "Error waiting for CAN frame: " << strerror(errno));
      return false;
    }
    else if (ret == 0)
    {
